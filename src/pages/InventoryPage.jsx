...
const InventoryPage = () => {
  ...
  const handleCostChange = (val) => {
    ...
    const autoExpected = (cost * 1.3).toFixed(2); // Soma 30%
    setEditingProduct({ 
      ...editingProduct, 
      cost_price: val, 
      expected_price: autoExpected 
    });
  };
  ...
  return (
    <>
                  ...
                  <tr key={p.id}>
                    ...
                    <td className="text-end">
                      <div className="d-flex justify-content-end gap-2">
                        ...
                        {/* Botão Excluir */}
                        <Button variant="outline-danger" size="sm" onClick={() => { setSelectedProduct(p); setShowDeleteModal(true); }}>
                          <Trash2 size={14}/>
                        </Button>
                        {/* BOTÃO EDITAR */}
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered size="sm">
        <Modal.Header closeButton className="bg-danger text-white"><Modal.Title className="h6">Confirmar Exclusão</Modal.Title></Modal.Header>
        <Modal.Body className="text-center py-4">
          Deseja realmente excluir <strong>{selectedProduct?.name}</strong>?<br/>
          <small className="text-muted text-uppercase mt-2 d-block">Esta ação não pode ser desfeita.</small>
        </Modal.Body>
        <Modal.Footer className="border-0 justify-content-center">
          <Button variant="secondary" onClick={() => setShowDeleteModal(false)}>Cancelar</Button>
          <Button variant="danger" onClick={handleDelete}>Excluir Agora</Button>
        </Modal.Footer>
      </Modal>
            ...
            <Row>
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>Categoria (Selecione ou Digite)</Form.Label>
                  <Form.Control 
